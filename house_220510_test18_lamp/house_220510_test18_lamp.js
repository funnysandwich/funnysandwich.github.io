...
let is_phone = false;
let time_rotate = 0;
let open_autoRotate = true;
let dragVar_flash = 0.0, dragVar = 0.0;
let open_follow = false;
let click_num = 0, click_time = 0;
let time_info = 0;
...
function setup() {
  ...
  for (let i=0; i<SIGN.length; i++) {
    SIGN[i] = createGraphics(50, 50);

    if (i == 0) {

      SIGN[i].background(255);
      SIGN[i].noStroke();
      SIGN[i].fill(0);
      SIGN[i].beginShape();
      for (let j=0; j<10; j++) {
        ...
        let y = 0;
        if (j<5) {
          x = cos(map(j, 0, 5-1, PI, TWO_PI)) * 3.5 + 2.2;
          y = sin(map(j, 0, 5-1, PI, TWO_PI)) * 3.5;
          y -= 10;
        } else {
          x = cos(map(j, 5, 10-1, 0, PI)) * 1.5 + 1.2;
          y = sin(map(j, 5, 10-1, 0, PI)) * 1.5;
          y += 4;
        }
        SIGN[i].vertex(x+SIGN[i].width/2, y+SIGN[i].height/2);
      }
      SIGN[i].endShape(CLOSE);
      SIGN[i].ellipse(SIGN[i].width/2+0.8, 39, 6, 6);
    } else if (i == 1) {
      SIGN[i].background(255);
      SIGN[i].noStroke();
      SIGN[i].fill(0);
      ...
      if (node_plane[node_plane.length-1] != null) {
        SIGN[i].beginShape();
        for (let j=0; j<node_plane.length; j++) {
          SIGN[i].vertex(node_plane[j].x, node_plane[j].y);
        }
        SIGN[i].endShape(CLOSE);
      } else {
...
function planeLoaded(str_plane) {
  ...
  for (let i=0; i<str_plane.length; i++) {
    ...
    SIGN[2].vertex(node_plane[i].x, node_plane[i].y);
  }
  SIGN[2].endShape(CLOSE);
}
...
function draw() {
  ...
  if (is_phone) {
    if (open_autoRotate) {
      ...
      let s_roY = map(time_rotate%720, 0, 720, 0, TWO_PI);
      if (time_rotate%720 == 0) {
        s_roY = 0;
        if (roY>0) {
          roY = (roY%TWO_PI) - TWO_PI;
        } else { 
          roY = (roY%TWO_PI);
        }
      }
      roY = easing_x(roY, s_roY, 0.1);
    } else {
      dragVar_flash = constrain(dragVar_flash, -50, 50);
      if (open_follow) {
        roY += dragVar_flash*0.03;
      } else {
        dragVar_flash = dragVar_flash%TWO_PI;
        dragVar_flash = easing_x(dragVar_flash, 0, 0.5);
        if (abs(dragVar_flash) < 0.1) {
          dragVar_flash = 0;
          if (time_follow<120) {
            time_follow ++;
          } else {
            open_autoRotate = true;
            roY = roY%TWO_PI;
            time_rotate = floor(map(roY, 0, TWO_PI, 0, 720));
          }
        }
        roY += dragVar_flash*0.01;
      }
    }
  } else {
  ...
  if (lamp.open_shine) {
    var_bkg = easing_x(var_bkg, 0, 0.8);
    var_white = easing_x(var_white, 0, 0.2);
  } else {
    var_bkg = easing_x(var_bkg, 1, 0.2);
    var_white = easing_x(var_white, 1, 0.6);
  }
  c_bkg = lerpColor(color(0), color(255), var_bkg);
  c_white = lerpColor(color(0), color(255), var_white);
...
function touchMoved() {
  if (is_phone) {
    ...
    open_follow = true;
    open_autoRotate = false;
    time_follow = 0;


    dragVar_flash = mouseX - dragVar;
    dragVar = mouseX;
  }
  return false;
}
