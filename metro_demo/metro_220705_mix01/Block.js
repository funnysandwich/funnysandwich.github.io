function Block(begin, W, D, index_z, index, isHouse, isRiver, isMountain, isSea, isRail, isNightMarket) {
  ...
  if (this.isHouse) {
    ...
    this.is_orientHouse = new Array(this.num_house);
    ...
    if (this.index_z != 0  ||  random(1) < stateVar_floor[state_floor][0]) {
      ...
      for (let i=0; i<this.num_house; i++) {
        ...
        } else if (var_rate < real_rate_normalHouse + real_rate_constrHouse + real_rate_catHouse + real_rate_CTowerHouse + real_rate_firewatchHouse + real_rate_blockHouse + real_rate_dottedHouse + real_rate_UGHouse + real_rate_orientHouse) {
          this.is_orientHouse[i] = true;
        }
        ...
        if (this.num_house == 1) {
          ...
          if (this.is_CTowerHouse[i]) {
            ...
            this.W_houses[i] *= 0.5;
            d[i] *= 0.5;
          } else if (this.is_orientHouse[i]) {
            this.W_houses[i] = min(this.W_houses[i], real(350));
            d[i] = this.W_houses[i];
          }
      ...
      for (let i=0; i<this.num_house; i++) {
        if (this.num_house == 1) {
          ...
          if (this.is_constrHouse[i]) {
            ...
            this.houses.push(new OrientHouse(be[i], this.W_houses[0], d[i], H_floor, floorNum[i], this.index_z, this.index));
          } else {
  ...
  this.change_shine = function() {
    if (have_button_shine) {

      if (this.isHouse) {
        if (this.houses.length > 0) {
          for (let i=0; i<this.houses.length; i++) {
            if (this.houses[i].is_normalHouse || this.houses[i].is_cat || this.houses[i].is_CTower || this.houses[i].is_firewatch || this.houses[i].is_orient) {
  ...
  this.change_submerge = function() {
    if (have_button_submerge) {
      if (this.isHouse) {
        if (this.houses.length > 0) {
          for (let i=0; i<this.houses.length; i++) {
            if (this.houses[i].is_normalHouse  ||  this.houses[i].is_constr || this.houses[i].is_cat || this.houses[i].is_CTower || this.houses[i].is_firewatch || this.houses[i].is_buildingBlock || this.houses[i].is_dottedLine || this.houses[i].is_orient) {
  ...
  this.change_spring = function() {
    if (have_button_spring) {
      if (this.isHouse) {
        if (this.houses.length > 0) {
          for (let i=0; i<this.houses.length; i++) {
            if (this.houses[i].is_normalHouse  ||  this.houses[i].is_constr || this.houses[i].is_cat || this.houses[i].is_CTower || this.houses[i].is_firewatch || this.houses[i].is_buildingBlock || this.houses[i].is_dottedLine || this.houses[i].is_underGround || this.houses[i].is_orient) {
  ...
  this.update = function(index) {
    ...
    if (this.isHouse) {
      if (this.houses.length > 0) {
        for (let i=0; i<this.houses.length; i++) {
          if (!this.isRiver  &&  (this.houses[i].is_normalHouse || this.houses[i].is_constr || this.houses[i].is_cat || this.houses[i].is_firewatch || this.houses[i].is_orient)  &&  this.houses[i].have_ladder) {
            if (this.houses.length==1  ||  i==1) {
              if (this.index > 5                                &&
                ...
                blocks[this.index-6].houses[0].is_orient)    &&
    ...
    if (!this.is_coverRoadHor) {
      for (let i=0; i<this.node_lampB.length; i++) {
        ...
        if (have_button_spring) {
          this.H_lampB[i] += this.H_add_spring;
        }
    ...
    if (index_z != 0  &&  this.index%6!=0  &&  !blocks[this.index-1].is_coverRoadHor) {
      for (let i=0; i<this.node_lampF.length; i++) {
        ...
        if (have_button_spring) {
          this.H_lampF[i] += this.H_add_spring;
        }
