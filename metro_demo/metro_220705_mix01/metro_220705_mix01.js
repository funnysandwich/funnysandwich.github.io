...
let count_songs = 1;//0
let totalSongs = 1;//0
let is_song_load_done = true;//false
let open_play = false;
...
function setup() {
  ...
  if (varData.split01.color == 0) {
    ...
    state_color = 1;
    //loadingSound(loadingPath + "blocktrain_yellow.mp3");
  ...
  loadingPath = "https://funnysandwich.github.io/metro_demo/split02/";
  if (varData.split02.landscape == 0) {
    //loadingSound(loadingPath + "blocktrain_landscape_drone.mp3");
  } else if (varData.split02.landscape == 1) {
    open_desert = true;
    //loadingSound(loadingPath + "blocktrain_landscape_desert.mp3");
  } else if (varData.split02.landscape == 2) {
    open_flood = true;
    //loadingSound(loadingPath + "blocktrain_landscape_reverse.mp3");
  } else if (varData.split02.landscape == 3) {
    open_sea = true;
    //loadingSound(loadingPath + "blocktrain_landscape_sea.mp3");
  } else if (varData.split02.landscape == 4) {
    open_mountain = true;
    //loadingSound(loadingPath + "blocktrain_landscape_mountain.mp3");
  }
  ...
  loadingPath = "https://funnysandwich.github.io/metro_demo/split04/";
  ...
  if (varData.split04.weather.weather_rain == 1) {
    open_rain = true;
    //totalSongs += 1;
    //loadingSound(loadingPath + "blocktrain_rain.mp3");
  }
  if (varData.split04.weather.weather_waterdrop == 1) {
    open_waterDrop = true;
    //totalSongs += 1;
    //loadingSound(loadingPath + "blocktrain_dew.mp3");
  }
  if (varData.split04.weather.weather_fog == 1) {
    open_fog = true;
    //totalSongs += 1;
    //loadingSound(loadingPath + "blocktrain_mist.mp3");
  }
  ...
  for (let j=0; j<6; j++) {
    ...
    roadVer.push(new RoadVer(createVector(x_begin-speed*0, skyline.y, 0), gap_block, -skyline.z, 0));
  }
  ...
  if (open_crucifix || open_roof || open_handrail || open_billboard || open_billboardRoof || open_ladder) {
    str_info.push("------");
    if (open_roof) {
  ...
  if (open_rain || open_waterDrop || open_fog) {
    str_info.push("------");
  ...
  if (have_button_stop  ||  have_button_sprint  ||  have_button_jump  ||  have_button_regenerate  ||  have_button_shine  ||  have_button_submerge  ||  have_button_spring) {
    str_info.push("------");
  ...
  if (is_song_load_done  &&  open_play) {//------- loadSongs -------
    ...
    if (!open_info) {
      ...
      if (open_flood) {
        ...
        vertex(endLine, skyline.y-real(75*1.5), real(300), 1, 1);
        vertex(beginLine, skyline.y-real(75*1.5), real(300), 0, 1);
        endShape();
        fill(255);
      }
