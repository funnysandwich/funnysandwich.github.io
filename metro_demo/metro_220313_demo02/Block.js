function Block(begin, W, D, index_z, index, isHouse, isRiver, isMountain, isSea, isRail, isNightMarket) {
  ...
  if (this.isHouse) {
    ...
    if (this.index_z != 0  ||  random(1) < stateVar_floor[state_floor][0]) {
      ...
      for (let i=0; i<this.num_house; i++) {
        ...
        let sum = rate_normalHouse + rate_constrHouse + rate_catHouse + rate_CTowerHouse + rate_firewatchHouse + rate_blockHouse;
        sum = max(sum, 1);
  ...
  if (open_billMc  &&  !this.isRiver  &&  !this.isMountain  &&  !this.isSea  &&  !this.isRail  &&  !this.isNightMarket) {
    if (this.houses.length > 0) {
      if ((this.houses[0].begin.x-this.begin.x) > real(75)  &&  !this.have_billMc  &&  random(1) < rate_billMc) {
      ...
      if (((this.begin.x+this.W)-(this.houses[this.houses.length-1].begin.x+this.houses[this.houses.length-1].W)) > real(75)  &&  !this.have_billMc  &&  random(1) < rate_billMc) {
        this.have_billMc = true;
        let x = random(this.houses[this.houses.length-1].begin.x+this.houses[this.houses.length-1].W, (this.begin.x+this.W)-real(75));
        let z = random(this.begin.z, this.begin.z+this.D-real(75));
        this.billMc = new Bill_Mc(createVector(x, this.begin.y, z), this.index_z);
      }
      if (this.houses.length == 2) {
        if ((this.houses[1].begin.x-(this.houses[0].begin.x+this.houses[0].W)) > real(75)  &&  !this.have_billMc  &&  random(1) < rate_billMc) {
          this.have_billMc = true;
          let x = random(this.houses[0].begin.x+this.houses[0].W, this.houses[1].begin.x-real(75));
          let z = random(this.begin.z, this.begin.z+this.D-real(75));
          this.billMc = new Bill_Mc(createVector(x, this.begin.y, z), this.index_z);
        }
      }
  ...
  this.change = function() {
    ...
    if (state_click == 0) {
      ...
      if (this.isHouse) {
        if (this.houses.length > 0) {
          for (let i=0; i<this.houses.length; i++) {
            if (this.houses[i].is_normalHouse  ||  this.houses[i].is_constr || this.houses[i].is_cat || this.houses[i].is_CTower || this.houses[i].is_firewatch || this.houses[i].is_buildingBlock) {
  ...
  this.display = function() {
    ...
    if ((this.index_z == 0  &&  this.begin.x < real(550)  &&  this.begin.x+this.W > real(-550))       ||  
      ...
      if (this.isNightMarket) {
        ...
        for (let i=0; i<this.node_gate.length; i++) {
          if (this.show_gate[i]) {
            vertex(this.node_gate[i][1].x-real(2), this.node_gate[i][1].y, this.node_gate[i][1].z);
            vertex(this.node_gate[i][1].x+real(2), this.node_gate[i][1].y, this.node_gate[i][1].z);
            vertex(this.node_gate[i][0].x+real(2), this.node_gate[i][0].y, this.node_gate[i][0].z);
            vertex(this.node_gate[i][0].x+real(2), this.node_gate[i][0].y, this.node_gate[i][0].z);
            vertex(this.node_gate[i][0].x-real(2), this.node_gate[i][0].y, this.node_gate[i][0].z);
            vertex(this.node_gate[i][1].x-real(2), this.node_gate[i][1].y, this.node_gate[i][1].z);

            vertex(this.node_gate[i][3].x-real(2), this.node_gate[i][3].y, this.node_gate[i][3].z);
            vertex(this.node_gate[i][3].x+real(2), this.node_gate[i][3].y, this.node_gate[i][3].z);
            vertex(this.node_gate[i][2].x+real(2), this.node_gate[i][2].y, this.node_gate[i][2].z);
            vertex(this.node_gate[i][2].x+real(2), this.node_gate[i][2].y, this.node_gate[i][2].z);
            vertex(this.node_gate[i][2].x-real(2), this.node_gate[i][2].y, this.node_gate[i][2].z);
            vertex(this.node_gate[i][3].x-real(2), this.node_gate[i][3].y, this.node_gate[i][3].z);

            for (let j=0; j<this.node_gate_rope[i].length-1; j++) {
              vertex(this.node_gate_rope[i][j].x, this.node_gate_rope[i][j].y, this.node_gate_rope[i][j].z);
              vertex(this.node_gate_rope[i][j+1].x, this.node_gate_rope[i][j+1].y, this.node_gate_rope[i][j+1].z);
              vertex(this.node_gate_rope[i][j+1].x, this.node_gate_rope[i][j+1].y+real(3), this.node_gate_rope[i][j+1].z);
              vertex(this.node_gate_rope[i][j+1].x, this.node_gate_rope[i][j+1].y+real(3), this.node_gate_rope[i][j+1].z);
              vertex(this.node_gate_rope[i][j].x, this.node_gate_rope[i][j].y+real(3), this.node_gate_rope[i][j].z);
              vertex(this.node_gate_rope[i][j].x, this.node_gate_rope[i][j].y, this.node_gate_rope[i][j].z);
              ...
              vertex(this.node_gate_rope[i][j].x, this.node_gate_rope[i][j].y, this.node_gate_rope[i][j].z);
              vertex(m.x, m.y, m.z);
              vertex(m_m_l.x, m_m_l.y+real(20), m_m_l.z);
              vertex(this.node_gate_rope[i][j+1].x, this.node_gate_rope[i][j+1].y, this.node_gate_rope[i][j+1].z);
              vertex(m.x, m.y, m.z);
              vertex(m_m_r.x, m_m_r.y+real(20), m_m_r.z);
            }
          }
        }
        endShape();
      ...
      if (!this.is_coverRoadHor) {
        ...
        t = constrain(map(this.node_lampB[0][0].z, skyline.z, 0, 1, 0), 0, 1);
      ...
      if (index_z != 0  &&  this.index%6!=0  &&  !blocks[this.index-1].is_coverRoadHor) {
        ...
        t = constrain(map(this.node_lampF[0][0].z, skyline.z, 0, 1, 0), 0, 1);
        stroke(lerpColor(c_near, c_far, t));
        beginShape(LINES);
        for (let i=0; i<this.node_lampF.length; i++) {
  ...
  this.displayInfo = function() {
    if ((this.index_z == 0  &&  this.begin.x < real(550)  &&  this.begin.x+this.W > real(-550))       ||  
      ...
      if (this.isSea) {
        if (this.isSkerry) {
          this.skerry.displayInfo();
        }
        if (this.index_z == 5) {
          this.sea.displayInfo();
        }
      }
      ...
      if (this.isNightMarket) {
        ...
        for (let i=0; i<this.node_gate.length; i++) {
          if (this.show_gate[i]) {
            vertex(this.node_gate[i][0].x, this.node_gate[i][0].y, this.node_gate[i][0].z);
            vertex(this.node_gate[i][1].x, this.node_gate[i][1].y, this.node_gate[i][1].z);
            vertex(this.node_gate[i][2].x, this.node_gate[i][2].y, this.node_gate[i][2].z);
            vertex(this.node_gate[i][3].x, this.node_gate[i][3].y, this.node_gate[i][3].z);
            for (let j=0; j<this.node_gate_rope[i].length-1; j++) {
              vertex(this.node_gate_rope[i][j].x, this.node_gate_rope[i][j].y, this.node_gate_rope[i][j].z);
              vertex(this.node_gate_rope[i][j+1].x, this.node_gate_rope[i][j+1].y, this.node_gate_rope[i][j+1].z);
            }
