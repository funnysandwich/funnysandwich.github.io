function LightRail(index_z) {
  ...
  this.displayInfo = function() {
    ...
    for (let i=0; i<this.node_ground.length; i++) {
      for (let j=0; j<this.node_ground[i].length; j++) {
        vertex(this.node_ground[i][j].x, this.node_ground[i][j].y, this.node_ground[i][j].z);
        vertex(this.node_ground[i][(j+1)%4].x, this.node_ground[i][(j+1)%4].y, this.node_ground[i][(j+1)%4].z);
      }
    }
    endShape();
