...
function setup() {
  ...
  state_winDecoration = 0; //0:无 1:潜水艇 2:遮光板 3:卷帘窗 4:窗帘 5:坏线框
    ...
    if (blocks.length > 0) {
      for (let i=0; i<blocks.length; i++) {
        if ((blocks[i].index_z == 0  &&  blocks[i].begin.x < real(550)  &&  blocks[i].begin.x+blocks[i].W > real(-550))       ||  
          ...
          if (blocks[i].blockMiniVer.length > 0) {
            for (let j=0; j<blocks[i].blockMiniVer.length; j++) {
              blocks[i].blockMiniVer[j].display();
            }
          }
        ...
        if (open_handrail) {
          if (blocks[i].isHouse) {
            for (let ii=0; ii<blocks[i].houses.length; ii++) {
              if ((blocks[i].houses[ii].is_normalHouse || blocks[i].houses[ii].is_firewatch)  &&  blocks[i].houses[ii].have_handrail) {
