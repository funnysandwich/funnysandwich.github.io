function Meisoul() {
  ...
  this.display = function() {
    ...
    for (let i=0; i<this.node_E.length-1; i++) {
      for (let j=0; j<this.node_E[i].length; j++) {
        ...
        //------ 少画美兽后背 ------
        let s = this.node_E[i][j].copy();
        s = PRotateY(s, -roY);
        let s_ = this.node_E[i][(j+1) %this.node_E[i].length].copy();
        s_ = PRotateY(s_, -roY);
        ...
        if (s.z>=0 || s_.z>=0) {
          if (i==3 && j==0) {
            ...
            fill(0);
            beginShape();
            vertex(this.node_E[i][j].x, this.node_E[i][j].y, this.node_E[i][j].z-real(0.1));
            vertex(this.node_E[i+1][j].x, this.node_E[i+1][j].y, this.node_E[i+1][j].z-real(0.1));
            vertex(this.node_E[i+1][(j+1)%this.node_E[i].length].x, this.node_E[i+1][(j+1)%this.node_E[i].length].y, this.node_E[i+1][(j+1)%this.node_E[i].length].z-real(0.1));
            vertex(this.node_E[i][(j+1)%this.node_E[i].length].x, this.node_E[i][(j+1)%this.node_E[i].length].y, this.node_E[i][(j+1)%this.node_E[i].length].z-real(0.1));
            endShape(CLOSE);
          } else if (i==3 && j==1) {
            beginShape();
            texture(EYE);
  ...
  this.displayInfo = function() {
    ...
    if (this.open_fast) {
      strokeWeight(real(0.85));
    } else {
      strokeWeight(real(0.45));
    }
    beginShape(LINES);
    for (let i=0; i<this.node_E_hand.length; i++) {
      for (let j=0; j<this.node_E_hand[i].length-1; j++) {
