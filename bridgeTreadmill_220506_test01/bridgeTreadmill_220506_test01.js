...
function setup() {
  ...
  if (open_info) {
    ...
    for (let i=0; i<mess.length; i++) {
      ...
      MESS.fill(90);
      if (mess[i].is_I) {
        MESS.text(nf(i, 2), MESS.width-30, y+25);
      } else {
        MESS.text(nf(i, 2), 16, y+25);
      }
      y += mess[i].H_all;
    }
    y = MESS.height/2.0;
...
function displayInfo_true() {
  ...
  PG.text("roY: "+roY, 10, 65);
  PG.text("mess.length: "+mess.length, 10, 80);
  PG.text("have_touch: "+have_touch, 10, 95 );
  PG.noStroke();
