function FirewatchHouse(begin, W, D, H, floor_num, index_z, index_block) {
  ...
  if (this.have_billboardRoof) {
    ...
    for (let i=0; i<this.node_billboardRoof.length; i++) {
      ...
      this.node_billboardRoof[i][2] = p5.Vector.sub(this.node_house[1][3], this.node_house[0][3]).setMag(this.H_billboardRoof).add(this.node_billboardRoof[i][0]);
  ...
  this.update = function(index_block) {
    ...
    for (let i=0; i<this.node_wall.length; i++) {
      ...
      for (let j=0; j<this.node_wall[i].length; j++) {
        ...
        n = PRotateX(n, this.node_rotate[i][1]);
        n.add(this.node[i]);
        this.node_wall[i][j] = easing_p(this.node_wall[i][j], n, 0.75);
      }
    }
