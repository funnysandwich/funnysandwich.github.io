function ConstrHouse(begin, W, D, H, floor_num, index_z, index_block) {
  ...
  if (this.have_billboardRoof) {
    this.state_billboardRoof = floor(random(0, 1.0));
    this.D_billboardRoof = constrain(this.D * random(0.25, 0.75), real(20), real(55));
    this.W_billboardRoof = constrain(this.W * random(0.15, 1), real(70), real(115));
    this.H_billboardRoof = real(random(55, 120));
    this.H_rate_billboardRoof_node1 = random(0.1, 0.65);
    if (random(1) < 1) {
      this.W_billboardRoof = min(this.W_billboardRoof, this.W);
    ...
    if (this.W > real(400)) {
      if (random(1) < 0.5) {
        this.X_billboardRoof = random(0, this.W/2.0 - this.W_billboardRoof);
      } else {
        this.X_billboardRoof = random(this.W/2.0, this.W - this.W_billboardRoof);
      }
    }
    ...
    for (let i=0; i<this.node_billboardRoof.length; i++) {
      const be = p5.Vector.sub(this.node_wall[this.node_wall.length-2][2], this.node_wall[this.node_wall.length-2][3]).setMag(this.X_billboardRoof).add(this.node_wall[this.node_wall.length-2][3]);

      this.node_billboardRoof[i][0] = p5.Vector.sub(this.node_wall[this.node_wall.length-2][2], be).setMag(map(i, 0, this.node_billboardRoof.length-1, 0, this.W_billboardRoof)).add(be);
      ...
      this.node_billboardRoof[i][3] = p5.Vector.sub(this.node_wall[this.node_wall.length-2][0], this.node_wall[this.node_wall.length-2][3]).setMag(this.D_billboardRoof).add(this.node_billboardRoof[i][0]);
    }
  }
  ...
  this.display_TRIANGLES = function() {
    ...
    if (this.state_structure == 0) {
      for (let i=1; i<this.node_wall.length-1; i++) {
        ...
        TRIANGLES_getRect_fill4(this.node_wall[i][0], this.node_wall[i][1], this.node_wall[i][2], this.node_wall[i][3], c1, c1, c2, c2);
      }
    } else if (this.state_structure == 1) {
