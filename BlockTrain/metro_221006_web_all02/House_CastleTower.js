function CastleTowerHouse(begin, W, D, H, floor_num, index_z, index_block, have_CWall, lastCTHouse_floorNum, lastCTHouse_node0) {
  ...
  if (this.have_billboard) {
    this.num_billboard = min(floor(random(1, this.floor_num-2)), 4);
    let count_floor = [];
    for (let i=1; i<this.floor_num-1; i++) {
      count_floor.push(i);
    }
    ...
    for (let i=0; i<this.num_billboard; i++) {
      ...
      this.W_billboard[i] = p5.Vector.dist(this.node_wall[this.whichFloor_billboard[i]][2], this.node_wall[this.whichFloor_billboard[i]][3]) * random(0.75, 0.95);
      this.H_billboard[i] = random(this.H_ori*0.35, this.H_ori*0.8);
      this.X_billboard[i] = random(0, p5.Vector.dist(this.node_wall[this.whichFloor_billboard[i]][2], this.node_wall[this.whichFloor_billboard[i]][3]) - this.W_billboard[i]);
