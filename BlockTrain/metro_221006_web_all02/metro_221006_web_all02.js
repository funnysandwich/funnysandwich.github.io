...
function setup() {
  ...
  speed = 0;
  mileage = 0;
  mileage_end = real(random(15000, 20000)*4);
  Y_shake = 0;
  ...
  //----------------------------------------⬇ Phase03 ⬇---------------------------------------
  //--------------------------------------------------------------------------------------------
  state_object_first = varData.Phase03.Object_First;
  //state_object_first = countProb([0, 4, 4, 3, 4, 4, 4, 6, 6, 6, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 2.5, 2.5, 4, 5, 5]);
  state_object_first = Math.min(Math.max(Math.floor(state_object_first), 0), 24);

  state_object_second = varData.Phase03.Object_Second;
  //state_object_second = countProb([0, 4, 4, 3, 4, 4, 4, 6, 6, 6, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 2.5, 2.5, 4, 5, 5]);
  state_object_second = Math.min(Math.max(Math.floor(state_object_second), 0), 24);

  state_object_third = varData.Phase03.Object_Third;
  //state_object_third = countProb([0, 4, 4, 3, 4, 4, 4, 6, 6, 6, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 2.5, 2.5, 4, 5, 5]);
  state_object_third = Math.min(Math.max(Math.floor(state_object_third), 0), 24);
  ...
  if (open_roof  ||  open_handrail) {
    totalSongs += 1;
    loadingSound_object(loadingPath_pre + loadingPath + "P0.mp3");
  }  
  if (open_billTezos) {
    totalSongs += 1;
    loadingSound_object(loadingPath_pre + loadingPath + "P1.mp3");
  }  
  if (open_billBitcoin) {
    totalSongs += 1;
    loadingSound_object(loadingPath_pre + loadingPath + "P2.mp3");
  }  
  if (open_billEthereum) {
    totalSongs += 1;
    loadingSound_object(loadingPath_pre + loadingPath + "P3.mp3");
  }  
  if (open_crucifix) {
    totalSongs += 1;
    loadingSound_object(loadingPath_pre + loadingPath + "P4.mp3");
  }  
  if (open_billMc  ||  open_billMessage  ||  open_billCannotReturn  ||  open_billCall  ||  open_billPower  ||  open_billMetro  ||  open_billFile) {
    totalSongs += 1;
    loadingSound_object(loadingPath_pre + loadingPath + "P5.mp3");
  }  
  if (open_billYoubetu  ||  open_billLike  ||  open_billTelegram) {
    totalSongs += 1;
    loadingSound_object(loadingPath_pre + loadingPath + "P6.mp3");
  }  
  if (open_parking  ||  open_fair  ||  open_tree  ||  open_gazebo  ||  open_paifang) {
    totalSongs += 1;
    loadingSound_object(loadingPath_pre + loadingPath + "P7.mp3");
  }  
  if (open_ladder  ||  open_PTTower) {
    totalSongs += 1;
    loadingSound_object(loadingPath_pre + loadingPath + "P8.mp3");
  }
  ...
  state_weather = varData.Phase04.Weather;
  // state_weather = countProb([10.5, 8, 8, 8, 8, 8, 8, 8, 8, 6, 7, 3, 3, 3, 3, 3, 2, 1.5]);
  state_weather = Math.min(Math.max(Math.floor(state_weather), 0), 17);

  if (state_weather == 0) {
  } else if (state_weather == 1) {
    open_rain = true;
    totalSongs += 1;
    loadingSound(loadingPath_pre + loadingPath + "W1.mp3");
  } else if (state_weather == 2) {
    open_waterDrop = true;
    totalSongs += 1;
    loadingSound(loadingPath_pre + loadingPath + "W2.mp3");
  } else if (state_weather == 3) {
    open_fog = true;
    totalSongs += 1;
    loadingSound(loadingPath_pre + loadingPath + "W3.mp3");
  } else if (state_weather == 4) {
    open_darkCloud = true;
    this.song_thunder;
    this.time_thunder = 0;
    this.time_max_thunder = 120;
    this.gap_thunder = 0;
    this.gap_max_thunder = floor(random(200, 500)*3);
    totalSongs += 1;
    loadingSound_thunder(loadingPath_pre + loadingPath + "W4.mp3");
  } else if (state_weather == 5) {
    open_sunBeam = true;
    totalSongs += 1;
    loadingSound(loadingPath_pre + loadingPath + "W5.mp3");
  } else if (state_weather == 6) {
    open_typhoon = true;
    totalSongs += 1;
    loadingSound(loadingPath_pre + loadingPath + "W6.mp3");
  } else if (state_weather == 7) {
    open_tornado = true;
    this.song_tornado = [];//new Array(3);
    totalSongs += 3;
    loadingSound_tornado(loadingPath_pre + loadingPath + "W7.mp3");
    loadingSound_tornado(loadingPath_pre + loadingPath + "W7.mp3");
    loadingSound_tornado(loadingPath_pre + loadingPath + "W7.mp3");
  } else if (state_weather == 8) {
    open_snow = true;
    totalSongs += 1;
    loadingSound(loadingPath_pre + loadingPath + "W8.mp3");
  } else if (state_weather == 9) {
    open_firework = true;
    this.song_boom = [];
    totalSongs += 3;
  ...
  } else if (state_weather == 10) {
    open_hugeMoon = true;
    totalSongs += 1;
    loadingSound(loadingPath_pre + loadingPath + "W10.mp3");
  } else if (state_weather == 11) {
    open_rain = true;
    open_waterDrop = true;
    totalSongs += 1;
    loadingSound(loadingPath_pre + loadingPath + "W11.mp3");
  } else if (state_weather == 12) {
    open_rain = true;
    open_darkCloud = true;
    this.song_thunder;
    this.time_thunder = 0;
    ...
    this.gap_max_thunder = floor(random(200, 500)*3);
    totalSongs += 1;
    loadingSound(loadingPath_pre + loadingPath + "W12.mp3");
  } else if (state_weather == 13) {
    open_snow = true;
    open_sunBeam = true;
    this.gap_max_thunder = floor(random(200, 500)*3);
    totalSongs += 1;
    loadingSound(loadingPath_pre + loadingPath + "W13.mp3");
  } else if (state_weather == 14) {
    open_snow = true;
    open_fog = true;
    totalSongs += 1;
    loadingSound(loadingPath_pre + loadingPath + "W14.mp3");
  } else if (state_weather == 15) {
    open_snow = true;
    open_firework = true;
    this.song_boom = [];
    totalSongs += 4;
    ...
    loadingSound_boom(loadingPath_pre + "Bounces/EFX/MP3/boom1.mp3");
    loadingSound_boom(loadingPath_pre + "Bounces/EFX/MP3/boom2.mp3");
    loadingSound(loadingPath_pre + loadingPath + "W15.mp3");
  } else if (state_weather == 16) {
    open_typhoon = true;
    open_waterDrop = true;
    totalSongs += 1;
    loadingSound(loadingPath_pre + loadingPath + "W16.mp3");
  } else if (state_weather == 17) {
    open_rain = true;
    open_fog = true;
    open_waterDrop = true;
    totalSongs += 1;
    loadingSound(loadingPath_pre + loadingPath + "W17.mp3");
  }
  ...
  BILL = Array.from(Array(3), () => new Array(1));
  for (let i=0; i<BILL.length; i++) {
    if (i == 0) {
      BILL[i][0] = createGraphics(23*dpi, 23*dpi);
    } else if (i == 1) {
      BILL[i][0] = createGraphics(39*dpi, 13*dpi);
    } else if (i == 2) {
      BILL[i][0] = createGraphics(13*dpi, 39*dpi);
    }
    BILL[i][0].background(addAlphaToColor(c_far, 160));
    BILL[i][0].noStroke();
    BILL[i][0].fill(addAlphaToColor(c_far, 60));
    for (let j=0; j<floor(random(6, 20)); j++) {
      let w = random(BILL[i][0].width*0.05, BILL[i][0].width*0.25);
      let x = random(0, BILL[i][0].width-w);
      BILL[i][0].rect(x, 0, w, BILL[i][0].height);
    }
  }
  ...
  var_PB = new Array(9);
  for (let i=0; i<var_PB.length; i++) {
    loadStrings("./PixelBoard/px"+i+".txt", PBLoaded);
  }
  ...
  if (open_billTezos) {
    ...
    loadStrings("./node/node_billTezos.txt", tezosLoaded);
  }


  if (open_billLike) {
    this.node_like = Array.from(Array(16), () => new Array(2));
    loadStrings("./node/node_billLike.txt", likeLoaded);
  }

  if (open_billCannotReturn) {
    this.node_CTReturn = Array.from(Array(40), () => new Array(2));
    loadStrings("./node/node_billCannotReturn.txt", cannotReturnLoaded);
  }
  ...
  if (open_billCall) {
    ...
    loadStrings("./node/node_billCall.txt", callLoaded);
  }


  if (open_billPower) {
    this.node_power = Array.from(Array(12), () => new Array(2));
    loadStrings("./node/node_billPower.txt", powerLoaded);
  }


  if (open_billBitcoin) {
    this.node_bitcoin = Array.from(Array(38), () => new Array(2));
    loadStrings("./node/node_billBitcoin.txt", bitcoinLoaded);
  }


  if (open_billEthereum) {
    this.node_ethereum = Array.from(Array(9), () => new Array(2));
    loadStrings("./node/node_billEthereum.txt", ethereumLoaded);
  }
  ...
  if (open_billTelegram) {
    ...
    loadStrings("./node/node_billTelegram.txt", telegramLoaded);
  }


  if (open_billMetro) {
    this.node_metro = Array.from(Array(28), () => new Array(2));
    loadStrings("./node/node_billMetro.txt", metroLoaded);
  }


  if (open_billFile) {
    this.node_file = Array.from(Array(18), () => new Array(2));
    loadStrings("./node/node_billFile.txt", fileLoaded);
  }
  ...
  if (!open_rain && !open_waterDrop && !open_fog && !open_darkCloud && !open_sunBeam && !open_typhoon && !open_tornado && !open_snow && !open_firework && !open_hugeMoon) {
    ...
    if (state_weather == 1) {
      s_info.push("Weather: RAIN");
    } else if (state_weather == 2) {
      s_info.push("Weather: WATERDROP");
    } else if (state_weather == 3) {
      s_info.push("Weather: MIST");
    } else if (state_weather == 4) {
      s_info.push("Weather: DARKCLOUD");
    } else if (state_weather == 5) {
      s_info.push("Weather: SUNBEAM");
    } else if (state_weather == 6) {
      s_info.push("Weather: TYPHOON");
    } else if (state_weather == 7) {
      s_info.push("Weather: TORNADO");
    } else if (state_weather == 8) {
      s_info.push("Weather: SNOW");
    } else if (state_weather == 9) {
      s_info.push("Weather: FIREWORK");
    } else if (state_weather == 10) {
      s_info.push("Weather: HUGEMOON");
    } else if (state_weather == 11) {
      s_info.push("Weather: RAIN+WATERDROP");
    } else if (state_weather == 12) {
      s_info.push("Weather: RAIN+DARKCLOUD");
    } else if (state_weather == 13) {
      s_info.push("Weather: SNOW+SUNBEAM");
    } else if (state_weather == 14) {
      s_info.push("Weather: SNOW+MIST");
    } else if (state_weather == 15) {
      s_info.push("Weather: SNOW+FIREWORK");
    } else if (state_weather == 16) {
      s_info.push("Weather: TYPHOON+WATERDROP");
    } else if (state_weather == 17) {
      s_info.push("Weather: RAIN+MIST+WATERDROP");
    }
  }
    ...
    if (speed > 0) {
      if (open_songs) {
        ...
        if (!song_track.isPlaying()) {
          ...
          if (is_onTheGround) {
            song_track.setVolume(volume * 0.6);
          } else {
            song_track.setVolume(volume * constrain(map(cameraY, -real(450), -real(10), 1, 0.6), 0.6, 1));
          }
        }
        ...
        if (songs_object.length > 0) {
          for (let i=0; i<songs_object.length; i++) {
            if (!songs_object[i].isPlaying()) {
              songs_object[i].setVolume(0);
              songs_object[i].play();
            } else {
              songs_object[i].setVolume(volume*0.65);
            }
          }
        }
      } else {
      ...
      if (gap_thunder < gap_max_thunder) {
        ...
        if (time_thunder < time_max_thunder) {
          ...
          if (open_songs) {
            ...
            song_thunder.setVolume(0);
          }
        } else {
          gap_thunder = 0;
          gap_max_thunder = floor(random(200, 500)*3);
          time_thunder = 0;
