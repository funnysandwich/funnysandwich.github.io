function OrientHouse(begin, W, D, H, floor_num, index_z, index_block) {
  this.ran = random(-999, 999);
  this.begin = begin.copy();
  this.W = new Array(floor_num+1);
  this.W[0] = W;
  const rate_W = random(max(map(floor_num, 2, 7, 0.65, 0.35), 0.35), max(map(floor_num, 2, 7, 0.75, 0.55), 0.55));
  for (let i=1; i<this.W.length; i++) {
    this.W[i] = map(i, 0, this.W.length-1, W, W*rate_W);
  }
  this.D = new Array(floor_num+1);
  ...
  for (let i=0; i<this.node.length; i++) {
    this.node[i] = this.begin.copy().add(this.W[i]/2, 0, this.D[i]/2);
  }
  ...
  for (let i=0; i<this.node_wall.length; i++) {
    for (let j=0; j<this.node_wall[i].length; j++) {
      this.node_wall[i][j] = createVector(this.W[i]/2.0 * pow(-1, ceil(j%1.5)+1), 0, this.D[i]/2.0 * pow(-1, floor(j/2)+1)).add(this.begin);
    }
  }
  ...
  this.num_winF_eachFloor = round(random((this.W[0]-gap_win) / (this.W_win+gap_win) *0.5, (this.W[0]-gap_win) / (this.W_win+gap_win)));
  this.num_winL_eachFloor = round(random((this.D[0]-gap_win) / (this.W_win+gap_win) *0.5, (this.D[0]-gap_win) / (this.W_win+gap_win)));
  this.num_winR_eachFloor = round(random((this.D[0]-gap_win) / (this.W_win+gap_win) *0.5, (this.D[0]-gap_win) / (this.W_win+gap_win)));
  ...
  for (let i=0; i<this.node_winF.length; i++) {
    ...
    for (let j=0; j<this.node_winF[i].length; j++) {
      for (let k=0; k<this.node_winF[i][j].length; k++) {
        this.node_winF[i][j][k] = this.begin.copy().add(this.W[i]/2, 0, this.D[i]/2);
      }
    }
    for (let j=0; j<this.node_winL[i].length; j++) {
      for (let k=0; k<this.node_winL[i][j].length; k++) {
        this.node_winL[i][j][k] = this.begin.copy().add(this.W[i]/2, 0, this.D[i]/2);
      }
    }
    for (let j=0; j<this.node_winR[i].length; j++) {
      for (let k=0; k<this.node_winR[i][j].length; k++) {
        this.node_winR[i][j][k] = this.begin.copy().add(this.W[i]/2, 0, this.D[i]/2);
      }
    }
  }
  ...
  this.node_eaver = Array.from(Array(this.floor_num), () => new Array(4));
  this.rate_slope = random(0.7, 0.9);
  this.W_eaver = H_floor * random(0.5, 1.15);
  if (state_floor == 3) {
    this.rate_slope = random(0.3, 0.45);
    this.W_eaver = H_floor * random(0.45, 0.7);
  }
  for (let i=0; i<this.node_eaver.length; i++) {
    for (let j=0; j<this.node_eaver[i].length; j++) {
      this.node_eaver[i][j] = p5.Vector.sub(this.node[i+1], this.node[i]).mult(this.rate_slope).add(this.node[i+1]);
      this.node_eaver[i][j] = p5.Vector.sub(this.node_wall[i+1][j], this.node_eaver[i][j]).setMag(this.W_eaver).add(this.node_wall[i+1][j]);
    }
  }
  ...
  if (this.have_crucifix) {
    ...
    this.begin_cfx = createVector(random(-this.W[this.W.length-1]/2.0+this.W_cfx/2.0, this.W[this.W.length-1]/2.0-this.W_cfx/2.0), 0, random(0, this.D[this.D.length-1]/2.0));
  ...
  if (this.have_roof) {
    this.node_roof = new Array(4);
    this.H_roof = H_floor * random(1.0, 1.5);
    this.node_roof_top = p5.Vector.sub(this.node[this.node.length-1], this.node[this.node.length-2]).setMag(this.H_roof).add(this.node[this.node.length-1]);
    for (let i=0; i<this.node_roof.length; i++) {
      let roof_m = p5.Vector.sub(this.node[this.node.length-1], this.node[this.node.length-2]).mult(this.rate_slope).add(this.node[this.node.length-1]);
      this.node_roof[i] = p5.Vector.sub(roof_m, this.node_wall[this.node_wall.length-1][i]).mult(0.5).add(this.node_wall[this.node_wall.length-1][i]);
    }
  }
  ...
  if (this.have_handrail) {
    this.num_handrailL = constrain(round(map(this.D[this.D.length-1], real(100), real(500), 2, 5)), 1, 5);
    this.num_handrailF = constrain(round(map(this.W[this.W.length-1], real(100), real(500), 2, 5)), 1, 5);
  ...
  if (this.have_billboard) {
    this.num_billboard = min(floor(random(1, this.floor_num-1)), 4);
    let count_floor = [];
    ...
    for (let i=0; i<this.num_billboard; i++) {
      ...
      this.W_billboard[i] = min(max(this.H_billboard[i] * 3, real(100)), this.W[this.whichFloor_billboard[i]]*0.95);
      this.H_billboard[i] = this.W_billboard[i] / 3.0;
      this.X_billboard[i] = random(0, this.W[this.whichFloor_billboard[i]]-this.W_billboard[i]);
      this.Z_billboard[i] = real(random(2.5, 10));
      if (num_PB1 > 0) {
        this.which_BILL[i] = floor(random(1, num_PB1+1));
      } else {
        this.which_BILL[i] = 0;
      }
    }
  ...
  if (this.have_billboardRoof) {
    ...
    this.D_billboardRoof = constrain(this.D[this.D.length-1] * random(0.25, 0.75), real(20), real(55));
    this.W_billboardRoof = max(this.W[this.W.length-1] * random(0.15, 1), real(70));
    this.H_billboardRoof = real(random(55, 120));
    this.H_rate_billboardRoof_node1 = random(0.1, 0.65);
    if (random(1) < 0.8) {
      this.W_billboardRoof = min(this.W_billboardRoof, real(115));
      this.H_billboardRoof = this.W_billboardRoof*1.25;
      this.H_rate_billboardRoof_node1 = 0.25;
    } else {
      this.state_billboardRoof = 1;
    }
    if (num_PB0 > 0) {
      this.which_BILL_roof = floor(random(1, num_PB0 +1));
    } else {
      this.which_BILL_roof = 0;
    }    
    this.node_billboardRoof = Array.from(Array(max(floor(map(this.W_billboardRoof, real(70), real(500), 2, 8)), 2)), () => new Array(4));

    this.X_billboardRoof = random(0, this.W[this.W.length-1] - this.W_billboardRoof);
  ...
  if (this.have_ladder) {
    ...
    this.D_ladder = constrain(this.W[floor(this.W.length/2)] * random(0.25, 0.5), real(50), real(150));
  ...
  this.change = function(begin1, W, D, H, floor_num, index_block) {
    ...
    for (let i=0; i<this.node_wall.length; i++) {
      for (let j=0; j<this.node_wall[i].length; j++) {
        this.node_wall[i][j] = createVector(this.W[i]/2.0 * pow(-1, ceil(j%1.5)+1), 0, this.D[i]/2.0 * pow(-1, floor(j/2)+1)).add(this.begin).add(this.W[i]/2, 0, this.D[i]/2);
      }
    }
    ...
    this.num_winF_eachFloor = round(random((this.W[this.W.length-1]-gap_win) / (this.W_win+gap_win) *0.5, (this.W[this.W.length-1]-gap_win) / (this.W_win+gap_win)));
    this.num_winL_eachFloor = round(random((this.D[this.D.length-1]-gap_win) / (this.W_win+gap_win) *0.5, (this.D[this.D.length-1]-gap_win) / (this.W_win+gap_win)));
    this.num_winR_eachFloor = round(random((this.D[this.D.length-1]-gap_win) / (this.W_win+gap_win) *0.5, (this.D[this.D.length-1]-gap_win) / (this.W_win+gap_win)));
    ...
    if (this.have_ladder) {
      ...
      this.D_ladder = constrain(this.W[floor(this.W.length/2.0)] * random(0.25, 0.5), real(50), real(150));
      this.H_ladder = real(random(10, 25));
      this.node_ladderF = Array.from(Array(5), () => new Array(2));
      this.node_ladderB = Array.from(Array(5), () => new Array(2));
      for (let i=0; i<this.node_ladderF.length; i++) {
  ...
  this.update = function(index_block) {
    ...
    this.node[0] = easing_p(this.node[0], this.begin.copy().add(this.W[0]/2 + speed*2, 0, this.D[0]/2), this.var_easing1);
    ...
    for (let i=0; i<this.node_wall.length; i++) {
      for (let j=0; j<this.node_wall[i].length; j++) {
        let n = createVector(this.W[i]/2.0 * pow(-1, ceil(j%1.5)+1), 0, this.D[i]/2.0 * pow(-1, floor(j/2)+1));
        if (state_click == 4) {
          let l = sin(map(i, 0, this.node_wall.length-1, 0, HALF_PI*1.75));
          n = createVector((this.W[i]/2.0+this.W_add[i]*l) * pow(-1, ceil(j%1.5)+1), 0, (this.D[i]/2.0+this.D_add[i]*l) * pow(-1, floor(j/2)+1));
        }
        n = PRotateZ(n, this.node_rotate[i][0]);
        n = PRotateX(n, this.node_rotate[i][1]);
        n.add(this.node[i]);
  ...
  this.display = function() {
    ...
    if (this.have_billboard) {
      ...
      for (let i=0; i<this.node_billboard.length; i++) {
        beginShape();
        texture(BILL[1][this.which_BILL[i]]);
      ...
      //for (let i=0; i<this.node_billboard.length; i++) {
      //  beginShape();
      //  vertex(this.node_billboard[i][0].x, this.node_billboard[i][0].y, this.node_billboard[i][0].z);
      //  vertex(this.node_billboard[i][1].x, this.node_billboard[i][1].y, this.node_billboard[i][1].z);
      //  vertex(this.node_billboard[i][2].x, this.node_billboard[i][2].y, this.node_billboard[i][2].z);
      //  vertex(this.node_billboard[i][3].x, this.node_billboard[i][3].y, this.node_billboard[i][3].z);
      //  endShape(CLOSE);
      //}
    }
    ...
    if (this.have_billboardRoof) {
      if (this.state_billboardRoof == 0) {
        ...
        fill(255);
        beginShape();
        texture(BILL[0][this.which_BILL_roof]);
  ...
  this.display_TRIANGLES = function() {
    ...
    //------------- ⬇L⬇ -------------
    if (this.node_wall[0][3].x > -real(300)) {
      for (let i=0; i<this.node_wall.length-1; i++) {
    ...
    //------------- ⬇R⬇ -------------
    if (this.node_wall[0][2].x < real(300)) {
      for (let i=0; i<this.node_wall.length-1; i++) {
    ...
    for (let i=0; i<this.node_eaver.length; i++) {
      //------------- ⬇F_eaver⬇ -------------
      c1 = lerpColor(c_near, c_far, constrain(map(this.node_wall[i+1][3].z-real(100), skyline.z, 0, 1, 0), 0, 1));
      c2 = lerpColor(c_near, c_far, constrain(map(this.node_eaver[i][3].z-real(500), skyline.z, 0, 1, 0), 0, 1));
      TRIANGLES_getRect_fill4(this.node_wall[i+1][3], this.node_wall[i+1][2], this.node_eaver[i][2], this.node_eaver[i][3], c1, c1, c2, c2);
      //------------- ⬆F_eaver⬆ -------------
      //------------- ⬇L_eaver⬇ -------------
      c1 = lerpColor(c_near, c_far, constrain(map(this.node_wall[i+1][0].z-real(100), skyline.z, 0, 1, 0), 0, 1));
      c2 = lerpColor(c_near, c_far, constrain(map(this.node_wall[i+1][3].z-real(100), skyline.z, 0, 1, 0), 0, 1));
      let c3 = lerpColor(c_near, c_far, constrain(map(this.node_eaver[i][3].z-real(500), skyline.z, 0, 1, 0), 0, 1));
      let c4 = lerpColor(c_near, c_far, constrain(map(this.node_wall[i][0].z-real(500), skyline.z, 0, 1, 0), 0, 1));
      TRIANGLES_getRect_fill4(this.node_wall[i+1][0], this.node_wall[i+1][3], this.node_eaver[i][3], this.node_eaver[i][0], c1, c2, c3, c4);
      //------------- ⬆L_eaver⬆ -------------
      //------------- ⬇R_eaver⬇ -------------
      c1 = lerpColor(c_near, c_far, constrain(map(this.node_wall[i+1][2].z-real(100), skyline.z, 0, 1, 0), 0, 1));
      c2 = lerpColor(c_near, c_far, constrain(map(this.node_wall[i+1][1].z-real(100), skyline.z, 0, 1, 0), 0, 1));
      c3 = lerpColor(c_near, c_far, constrain(map(this.node_eaver[i][1].z-real(500), skyline.z, 0, 1, 0), 0, 1));
      c4 = lerpColor(c_near, c_far, constrain(map(this.node_wall[i][2].z-real(500), skyline.z, 0, 1, 0), 0, 1));
      TRIANGLES_getRect_fill4(this.node_wall[i+1][2], this.node_wall[i+1][1], this.node_eaver[i][1], this.node_eaver[i][2], c1, c2, c3, c4);
      //------------- ⬆R_eaver⬆ -------------
      //------------- ⬇B_eaver⬇ -------------
      c1 = lerpColor(c_near, c_far, constrain(map(this.node_wall[i+1][0].z-real(100), skyline.z, 0, 1, 0), 0, 1));
      c2 = lerpColor(c_near, c_far, constrain(map(this.node_eaver[i][0].z-real(500), skyline.z, 0, 1, 0), 0, 1));
      TRIANGLES_getRect_fill4(this.node_wall[i+1][0], this.node_wall[i+1][1], this.node_eaver[i][1], this.node_eaver[i][0], c1, c1, c2, c2);
      //------------- ⬆B_eaver⬆ -------------
    }
    ...
    if (this.have_roof) {
      ...
      for (let i=1; i<this.node_roof.length; i++) {

        c1 = lerpColor(c_near, c_far, constrain(map(this.node_roof_top.z + real(500), skyline.z, 0, 1, 0), 0, 1));
        c2 = lerpColor(c_near, c_far, constrain(map(this.node_roof[i].z, skyline.z, 0, 1, 0), 0, 1));
        let c3 = lerpColor(c_near, c_far, constrain(map(this.node_roof[(i+1)%4].z, skyline.z, 0, 1, 0), 0, 1));
        TRIANGLES_getTriangle_fill3(this.node_roof_top, this.node_roof[i], this.node_roof[(i+1)%4], c1, c2, c3);

        c1 = lerpColor(c_near, c_far, constrain(map(this.node_roof[i].z, skyline.z, 0, 1, 0), 0, 1));
        c2 = lerpColor(c_near, c_far, constrain(map(this.node_roof[(i+1)%4].z, skyline.z, 0, 1, 0), 0, 1));
        c3 = lerpColor(c_near, c_far, constrain(map(this.node_wall[this.node_wall.length-1][(i+1)%4].z - real(100), skyline.z, 0, 1, 0), 0, 1));
        let c4 = lerpColor(c_near, c_far, constrain(map(this.node_wall[this.node_wall.length-1][i].z - real(100), skyline.z, 0, 1, 0), 0, 1));
        TRIANGLES_getRect_fill4(this.node_roof[i], this.node_roof[(i+1)%4], this.node_wall[this.node_wall.length-1][(i+1)%4], this.node_wall[this.node_wall.length-1][i], c1, c2, c3, c4);
      }
    }
  ...
  this.displayInfo = function() {
    ...
    for (let i=0; i<this.node_eaver.length; i++) {
      for (let j=0; j<this.node_eaver[i].length; j++) {
        LINES_getLine(this.node_eaver[i][j], this.node_wall[i+1][j]);
        LINES_getLine(this.node_eaver[i][j], this.node_eaver[i][(j+1)%4]);
      }
    }
