function Tree(begin_room, W_room, index_z) {
  ...
  this.H_ori_target *= min(map(this.W_room, real(75), real(200), 0.5, 1.0), 1.0);
  if (state_floor == 3) {
  ...
  this.W *= min(map(this.W_room, real(75), real(200), 0.85, 1.0), 1.0);
  if (state_floor == 3) {
    this.W = real(random(45, 90)) * map(this.node.length, 4, 7, 0.75, 1);
  }
  this.begin = this.begin_room.copy().add(random(this.W/2.0+real(10), (this.W_room-this.W/2.0))-real(10), 0, random(this.W/2.0, (real(200)-this.W/2.0)));
  ...
  if (have_button_spring) {
    ...
    this.H_ori_ori = this.H_ori_target;
    this.H_ori = this.H_ori_target;
  } 
  ...
  if (this.have_foliage) {
    ...
    this.W_foliage_target *= min(map(this.W_room, real(75), real(200), 0.5, 1.0), 1.0);
  ...
  this.changeH = function() {
    if (!this.open_spring  &&  random(1) < 0.85) {
      ...
      //this.H_ori_ori = H_floor * random(map(stateVar_floor[state_floor][this.index_z*2+1], 2, 14, 0.55, 2.0), map(stateVar_floor[state_floor][this.index_z*2+2]+1, 2, 14, 0.75, 2.5));
      //this.H_ori_ori *= min(map(this.W_room, real(75), real(200), 0.5, 1.0), 1.0);
      //if (state_floor == 3) {
      //  this.H_ori_ori = H_floor * random(1.75, 2.5);
      //}
    }
  };
  ...
  this.update = function() {
    ...
    for (let i=2; i<this.node.length; i++) {
      let n = p5.Vector.sub(this.node[i-1], this.node[i-2]).mult(0.8);
