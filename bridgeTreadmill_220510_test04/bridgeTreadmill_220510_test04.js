/*
 ...
 没用！没用！没用！没用！没用！没用！没用！没用！
 
 最后发现，只要在 touch function 里加 return false 就行了！！！！！！！
...
function setup() {
  ...
  //viewer="tz1XuoTxu2m7Kdp5iBhLv35KxFcjkoeY5vYe";
  //viewer="tz1gsMqei64Bi4VZAetahn5EsX1v3aNqM2qY";
  //viewer = "tz1dv43Gehop2K1U5nWxNQhr2JBj9MxycAQj";//bb

  if (viewer!=null && viewer.localeCompare("false")==true) {
    fxhash=viewer;
    hasviewer=true;
    b58dec = (str) => str.split('').reduce((p, c, i) => p + alphabet.indexOf(c) * (Math.pow(alphabet.length, str.length-i-1)), 0);
  ...
  c_info = color(255);
  if (state_color == 0 || state_color == 3 || state_color == 5 || state_color == 7 || state_color == 9 || state_color == 10 || state_color == 11) {
    c_info = color(0);
  }


  //0:line 1:wechat 2:fb 3:iphone
  state_fillet = floor(random(0, 4));
  ...
  for (let i=0; i<mess.length; i++) {
    ...
    MESS.textSize(12);
    MESS.noStroke();
    MESS.fill(colorWithAlpha(c_info, 100));
    if (mess[i].is_I) {
...
function displayInfo() {
  ...
  PG.text("ColorMode: "+String.fromCharCode(65+state_color), x, y+gap*2);
  PG.text("Color_Bkg: "+c_bkg[state_color][0], x+35, y+gap*3);
  PG.text("Color_Y: "+c_bkg[state_color][2], x+35, y+gap*4);
  PG.text("Color_I: "+c_bkg[state_color][1], x+35, y+gap*5);
  PG.text("FilletMode: "+String.fromCharCode(65+state_fillet), x, y+gap*6);
  PG.text("Num_Message: "+mess.length, x, y+gap*7);
  let num_I = 0;
  for (let i=0; i<mess.length; i++) {
    if (mess[i].is_I) {
      num_I += 1;
  ...
  PG.text("Num_Message_Y: "+(mess.length-num_I), x, y+gap*8);
  PG.text("Num_Message_I: "+num_I, x, y+gap*9);
  PG.text("Speed: "+nfc(belt.speed*100*5, 2)+"/10.00", x, y+gap*10);
  PG.text("Rotate: "+nfc(roY, 2), x, y+gap*11);


  PG.stroke(colorWithAlpha(c_info, 128));
  PG.strokeWeight(1);
  PG.fill(c_bkg[state_color][0]);
  PG.rect(x, y+gap*3, 30, -8);
  PG.fill(c_bkg[state_color][2]);
  PG.rect(x, y+gap*4, 30, -8);
  PG.fill(c_bkg[state_color][1]);
  PG.rect(x, y+gap*5, 30, -8);

  //PG.noFill();
  //const w_fillet = 26.0/34.0 * mess[0].W_fillet;
  //PG.rect(x, y+gap*8, 60, -26, w_fillet);
  ...
  if (viewer!=null && viewer.localeCompare("false")) {
    PG.textAlign(CENTER);
    PG.textSize(13);
    PG.fill(colorWithAlpha(c_info, 160));
    PG.text("/ "+viewer+" /", PG.width/2, 20);
  }
  // }
}
