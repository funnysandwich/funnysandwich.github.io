function PCsetup() {
  ...
  link_fb = createA("https://www.facebook.com/KDLAF", "KDLAF", "_blank");
  link_fb.position(0, 0);
  link_fb.style('font-size', 12*max(scaleRate/1.072, 1));
  link_fb.style('color', '#b8b8b8');
  link_fb.hide();
...
function PCdraw() {
  ...
  pop();
  if (scrollVar > 0  &&  scrollVar <  beginY01 + sideW*(img_bar[0].height/img_bar[0].width)*0.5) {
    link_fb.show();
    link_fb.position(width-sideW +sideW*0.504, map(scrollVar, height, beginY01+height, beginY01*0.533, -beginY01+beginY01*0.533));
  } else {
    link_fb.hide();
  }
  //-------------------------------- ⬆首页侧边⬆ --------------------------------
  ...
  if (scrollVar > beginY01) {
    if (scrollVar  <  beginY01 + sideW*(img_bar[0].height/img_bar[0].width)) {
      image(img_bar[0], width-sideW, map(scrollVar, beginY01, beginY01+sideW*(img_bar[0].height/img_bar[0].width), -sideW*(img_bar[0].height/img_bar[0].width), 0), sideW, sideW*(img_bar[0].height/img_bar[0].width));
    } else {
  ...
  if (scrollVar < beginY01+height) {
    ...
    image(PG, 0, constrain(map(scrollVar, beginY01+height-sideW*(img_bar[0].height/img_bar[0].width), beginY01+height, 0, -300), -300, 0), width, ceil(width*(PG.height/PG.width)));
