...
let link_kdlaf2016, link_kdlaf2017, link_kdlaf2018, link_kdlaf2019, link_kdlaf2020, link_fb;
let open_mask2016, open_mask2017, open_mask2018, open_mask2019, open_mask2020;

let sideW;
let scaleRate, scaleWH, scalePG;
let wait_side;
let beginY00, beginY01, beginY02, beginY03, beginY04, beginY05;
let lightW;
...
let is_pc, is_wLongerThanH, open_homepage;
let is_touched;
...
function preload() {
  ...
  let str_loading = createDiv("loading...");
  ...
  str_loading.position(windowWidth/2 -50, windowHeight/2 + 94/2 +20);

  let img_loading00_DOM = createImg("data/img_loading00.png");
  let img_loading01_DOM = createImg("data/img_loading01.png");
  let img_loading02_DOM = createImg("data/img_loading02.png");    
  img_loading00_DOM.position(floor((windowWidth-94)/2), floor((windowHeight-94)/2));
  img_loading01_DOM.position(floor((windowWidth-94)/2), floor((windowHeight-94)/2 -94));
  img_loading02_DOM.position(floor((windowWidth-94)/2), floor((windowHeight-94)/2 -94 -94*1.35));
...
function setup() {
  ...
  a = createDiv("hahaha<br>haha<br>hahaha<br>halalallalala<br>lala<br>lalall<br>aaaa<br>aaaaaaa");
  a.position(100, 100);
  a.style('width', '200');
  a.style('height', '100');
...
function draw() {
  if (!open_homepage) {
    ...
    image(img_loading02, floor((width-img_loading02.width)/2), s_y02_loading);
    image(img_loading01, floor((width-img_loading01.width)/2), s_y01_loading);
    image(img_loading00, floor((width-img_loading00.width)/2), (height-img_loading00.height)/2);
...
function displayInfo() {
  ...
  text("is_wLongerThanH: "+is_wLongerThanH, 10, 50);
  ...
  text("side_wait: "+wait_side, 10, 335);
  text("------------", 10, 350);
  text("count_open_info: "+count_open_info, 10, 365);
  text("is_touched: "+is_touched, 10, 380);
...
function windowResized() {
  if (is_pc) {
    ...
    link_fb.style('font-size', 12*max(scaleRate/1.072, 1));
