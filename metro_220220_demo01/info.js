...
function displayInfo() {
  ...
  PG.textAlign(LEFT);
  PG.textSize(8);
  str_info[1] = ("Speed: " + nfc(speed, 2));
  str_info[2] = ("Mileage: "+mileage);
  for (let i=0; i<str_info.length; i++) {
    PG.text(str_info[i], x, y+gap*i);
  }
  ...
  for (let i=0; i<5; i++) {
    ...
    if (i == 0) {
      PG.fill(c_all[state_c_all[state_color][4]]);
    } else if (i == 1) {
      PG.fill(c_all[state_c_all[state_color][3]]);
    } else {
      PG.fill(c_all[state_c_all[state_color][i-2]]);
    }
