...
function preload() {
  song.push(loadSound('data/train_rain.wav'));
  song.push(loadSound('data/train_90.wav'));
  song.push(loadSound('data/train_alone.wav'));
}
...
document.onclick = function (event) {
  ...
  if (!song[index_song].isPlaying()) {
    song[index_song].play();
  }
...
function keyPressed() {
  if (key == ' ') {
    song[index_song].pause();
    index_song += 1;
    if (index_song >= song.length) {
      index_song = 0;
    }
    song[index_song].play();
  } else if (key == 'w' || key == 'W') {
