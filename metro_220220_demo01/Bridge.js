function Bridge(begin, W) {
  ...
  this.display = function() {
    ...
    for (let i=0; i<this.node_F.length-1; i++) {
      beginShape();
      let t = constrain(map(this.node_B[i].z, skyline.z, 0, 1, 0), 0, 1);
      fill(lerpColor(c_near, c_far, t));
      vertex(this.node_B[i].x, this.node_B[i].y, this.node_B[i].z);
      vertex(this.node_B[i+1].x, this.node_B[i+1].y, this.node_B[i+1].z);
      t = constrain(map(this.node_F[i].z, skyline.z, 0, 1, 0), 0, 1);
      fill(lerpColor(c_near, c_far, t));
      vertex(this.node_F[i+1].x, this.node_F[i+1].y, this.node_F[i+1].z);
    ...
    let t = constrain(map(this.node_F[0].z, skyline.z, 0, 1, 0), 0, 1);
    ...
    beginShape(LINES);
    for (let i=0; i<this.node_bdgPier.length; i++) {
      for (let j=0; j<this.node_bdgPier[i].length; j++) {
        vertex(this.node_bdgPier[i][j].x, this.node_bdgPier[i][j].y, this.node_bdgPier[i][j].z);
      }
    }
    endShape();
  };
