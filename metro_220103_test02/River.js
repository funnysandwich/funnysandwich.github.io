function River(begin, W_block) {
  ...
  if (this.have_PTTower) {
    ...
    for (let i=0; i<this.num_PTTower; i++) {
      ...
      this.pTTower[i] = new PTTower(createVector(x, skyline.y, z), w, h, i, this.num_PTTower);
    }
  ...
  this.display = function() {
    ...
    for (let i=0; i<this.node.length/2; i++) {
      let t = constrain(map(this.node[i].z, skyline.z, 0, 1, 0), 0, 1);
      fill(lerpColor(lerpColor(c_near, c_sky, 0.25), c_sky, t));
      vertex(this.node[i].x, this.node[i].y, this.node[i].z);
      t = constrain(map(this.node[i+this.node.length/2].z, skyline.z, 0, 1, 0), 0, 1);
      fill(lerpColor(lerpColor(c_near, c_sky, 0.25), c_sky, t));
