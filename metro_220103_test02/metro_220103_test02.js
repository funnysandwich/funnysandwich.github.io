...
function setup() {
  ...
  for (let i=0; i<6; i++) {
    ...
    blocks.push(new Block(createVector(beginLine, skyline.y, z), real(100)*5, real(100)*5, i, blocks.length, true, false, is_mountain, is_rail));
    count_blocks += 1;
    if (i<5) {
...
function draw() {
  ...
  if (abs(blocks[blocks.length-1].begin.x-beginLine) > w_next_block+gap_block) {
    ...
    for (let i=0; i<6; i++) {
      ...
      let is_house = true;
      if (isRiver_next  ||  is_rail) {
        is_house = false;
      }
      let d = real(100)*5;
      let z = map(i, 0, 6-1, -d, -d*6) - gap_block*i;
      blocks.push(new Block(createVector(beginLine + x_error, skyline.y, z), w_next_block, real(100)*5, i, blocks.length, is_house, isRiver_next, is_mountain, is_rail));
      count_blocks += 1;
    }
