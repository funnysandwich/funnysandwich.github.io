function Block(begin, W, D, index_z, index, isHouse, isRiver, isMountain, isRail) {
  ...
  this.isFair = false;
  if (open_fair  &&  !this.isRiver  &&  !this.isRail) {
    if (this.index_z == 0) {
      this.isFair = false;
      ...
      if (this.isFair) {
        ...
        for (let i=0; i<num_ver; i++) {
          for (let j=0; j<num_hor; j++) {
            const x = map(j, 0, num_hor, 0, this.W);
            const z = map(i, 0, num_ver, 0, this.D);
            this.popupTent[j + i*num_hor].change(this.begin.copy().add(x, 0, z), real(200));
          }
        }
      }
