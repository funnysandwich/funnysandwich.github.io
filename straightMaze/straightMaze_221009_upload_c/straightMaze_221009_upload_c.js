// include a comment about LICENSE.md in straightMaze_221009_upload_c.js
...
function setup() {
  ...
  //rate_door_left;
  //rate_door_right;
  rate_openDoor_left = 0;
  rate_openDoor_right = 0;
  rate_doorplate_left = 0.75; 
  rate_doorplate_right = 0.75;
  rate_toiletMale_left = 0.1;
  rate_toiletMale_right = 0.1;
  rate_toiletFemale_left = 0.1;
  rate_toiletFemale_right = 0.1;
  rate_power_left = 0.1;
  rate_power_right = 0.1;
  rate_stairs_left = 0;
  rate_stairs_right = 0;
...
function draw() {
  ...
  //------------------------------⬇ update ⬇--------------------------------------------------------------------------------------------------------------
  //--------------------------------------------------------------------------------------------------------------------------------------------------------
  Y_shake = map(sin(frameCount*0.25 * map(speed, 0, real(20), 0, 1)), -1, 1, -real(4), real(4));
  ...
  if (corridor2.length > 0) {
    for (let i=0; i<corridor2.length; i++) {
      if (corridor2[i].dead) {
        corridor2.splice(i, 1);
      }
    }
  }
  ...
  if (corridor2.length > 0) {
    for (let i=0; i<corridor2.length; i++) {
      corridor2[i].update(i);
    }
  }
  if (corridor3.length > 0) {
    for (let i=0; i<corridor3.length; i++) {
      if (corridor3[i].dead) {
        corridor3.splice(i, 1);
      }
    }
  }
  ...
  if (!open_info) {
    ...
    if (corridor2.length > 0) {
      for (let i=0; i<corridor2.length; i++) {
        if (corridor2[i].index_count%2 != 0) {
          corridor2[i].displayInfo();
        }
      }
    }
    if (corridor3.length > 0) {
      for (let i=0; i<corridor3.length; i++) {
        if (corridor3[i].index_count%2 != 0) {
          corridor3[i].displayInfo();
        }
      }
    }
    ...
    if (corridor2.length > 0) {
      ...
      let center = corridor2[corridor2.length-1].node_show[corridor2[corridor2.length-1].node_show.length-1].copy();
      LINES_getLine(createVector(-real(50)-real(750), -real(50), skyline.z), center);
      LINES_getLine(createVector(real(50)-real(750), -real(50), skyline.z), center);
      LINES_getLine(createVector(real(50)-real(750), real(50), skyline.z), center);
      LINES_getLine(createVector(-real(50)-real(750), real(50), skyline.z), center);
    }
    if (corridor3.length > 0) {
      ...
      let center = corridor3[corridor3.length-1].node_show[corridor3[corridor3.length-1].node_show.length-1].copy();
      LINES_getLine(createVector(-real(50)+real(750), -real(50), skyline.z), center);
      LINES_getLine(createVector(real(50)+real(750), -real(50), skyline.z), center);
      LINES_getLine(createVector(real(50)+real(750), real(50), skyline.z), center);
      LINES_getLine(createVector(-real(50)+real(750), real(50), skyline.z), center);
...
function realColor(n) {
  return lerpColor(c_far, c_near, constrain(map(sin(map(n.z, skyline.z, endLine-real(0), HALF_PI, PI)), 1, 0, 0, 1), 0, 1));
  //return lerpColor(c_far, c_near, constrain(map(n.z, skyline.z, endLine, 0, 1), 0, 1));
}
