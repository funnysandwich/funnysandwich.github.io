...
function Corridor(count, index, begin, index_corr) {
  this.index_count = count;
  this.index = index;


  this.W = real(map(noise(count*0.15+99+index_corr*99), 0, 1, 200, 400));
  this.H = real(map(noise(count*0.05+index_corr*99), 0, 1, 300, 1500));
  this.X_move = real(map(noise(count*0.05+999+index_corr*99), 0, 1, -this.W*0.35, this.W*0.35));
  this.D = real(500);
  this.dead = false;
  ...
  this.update = function(index) {
    ...
    } else if (index_corr == 2) {
      if (this.index != corridor2.length-1) {
        this.node[this.node.length-1] = corridor2[this.index+1].node[0].copy();
      }
    } else if (index_corr == 3) {
      if (this.index != corridor3.length-1) {
        this.node[this.node.length-1] = corridor3[this.index+1].node[0].copy();
      }
    }
